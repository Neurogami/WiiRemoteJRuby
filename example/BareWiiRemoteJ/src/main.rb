...
begin  
  ...
  add_to_load_path '../lib/ruby/swingset-local'
  add_to_load_path '/intro'
  add_to_load_path  '../lib/ruby/wiiremotejruby' 

  warn "Load paths:\n#{$:.join "\n"}"

  require 'pp'
  require 'wii_remote'
  require 'wiiremotejruby'
  require 'intro_ui'
  require 'intro_view'
  require 'intro_controller'

  IntroController.instance.open
rescue => e
